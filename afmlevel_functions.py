...
def remove_small_zeros(arr, min_size=50):
    ...
    small_labels = np.where(region_sizes < min_size)[0] + 1  # +1 because we skipped label 0
...
def applymodel_mask(imarray):
    ...
    n_channels = 1
    model_filename = 'unetmodel_Aire_Mask_80eps_b32_d0_1f7_f7_7l_line2_train34_otsublur1_epoch69' 
    ...
    class UNet(nn.Module):
        def __init__(self, n_channels): 
            ...
            self.final = nn.Conv2d(16, 1, 1) #go from 64 channels to 1 channel, kernel size 1
    ...
    predictedmask =  predict_on_image(loaded_model, imnorm, device) #shape [1, 256, 256]
    predictedmask =  predictedmask.squeeze(0)  #shape [256, 256]
    predictedmask = predictedmask.numpy()
    predictedmask_resize = cv2.resize(predictedmask, (imarray.shape[1], imarray.shape[0]), interpolation=cv2.INTER_NEAREST)
...
def applymodel_mask_stack(imarray):
    ...
    dim = (256,256) #dimensions for image resize
    if imarray.ndim ==3:
        originaldim = (imarray.shape[2], imarray.shape[1]) #have to use in this order for resize 
    else:
    ...
    # apply model to image       
    predictedmasklist = []
    for imnorm, minval, datarange in zip(imnormlist, minvallist, datarangelist):
        maskarray =  predict_on_image(loaded_model, imnorm, device) #shape [1, 256, 256]
        maskarray_2D =  maskarray.squeeze(0)  #shape [256, 256]
        maskarray_np = maskarray_2D.numpy() 
        
        maskarray_resize = cv2.resize(maskarray_np, originaldim, interpolation=cv2.INTER_NEAREST)
...
def applymodel_bg_pixelsplit_all(imarray, lineorder):
    ...
    n_channels = 1
    lineorder = lineorder
    model_filename = 'unetmodel_Aire_BG_60eps_b32_d0_1f9_f9_7l_MSE_0line_train34_epoch59' 
    #won't be changing which model is used once we have found the best one, so this is defined in the function
    ...
    if imarray.shape == (256, 256):
        ...
        arrays = [imnorm[0::2, 0::2], imnorm[0::2, 1::2], imnorm[1::2, 0::2], imnorm[1::2, 1::2]]
...
def applymodel_bg_pixelsplit_stack(imarray, lineorder):
        ...
        if imarrayi.shape == (256, 256):
            ...
            for i in range(hj):
                for j in range(wj):
                    array = mirror_all[i:hr:hj, j:wr:wj]            
                    array = array[:256, :256]  
                    arrays.append(array)  #now have arrays for each image in the stack rather than just 1 image
    ...
    predictedBGlist = []
    for arrays, minval, datarange in zip(arrayslist, minvallist, datarangelist):
        BGarrays =  predict_on_image(loaded_model, arrays, device) #shape [1, 256, 256]
        BGarrays_2D =  [array.squeeze(0) for array in BGarrays]  #shape [256, 256]
        BGarrays_np = [array.numpy() for array in BGarrays_2D] 
