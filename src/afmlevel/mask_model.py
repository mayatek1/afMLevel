...
This module implements the `ml_mask()` function, which applies a trained U-Net model
to predict a feature mask (1 = background, 0 = feature) of AFM images and then feeds
the output into mask based levelling routines through the `level_ml_mask()` function.
The public functions (ml_mask(), ml_edges() and level_ml_mask()) can handle
both single 2D images and 3D stacks.

* `ml_mask()` primary inference function. Preprocesses data by normalisaition and
  resizes frames to 256 x 256 then applied the mask model trained U-Net to predict a
  feature mask.
* `ml_edges()` takes the mask generated by `ml_mask()` and creates an edge mask by
  applying scikit-image morphology operations.
* `level_ml_mask()` uses the masks generated by `ml_mask()` and `ml_edges()` within
  pre-programmed automated levelling routines, these are set in `DEFAULT_ML_ROUTINES`.
  The function outputs either a 2D numpy array of the levelled image or a 3D stack
  depending on the input.

The `level_ml_mask()` function is the main entry point which takes an AFM image or
stack, applies the model to generate the mask and then uses this in routines to output
levelled AFM images and stacks.
